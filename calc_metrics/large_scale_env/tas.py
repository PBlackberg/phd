...
def load_data(switch, source, dataset, experiment):
    da = cF.var3D if switch['constructed_fields'] else None
    da = xr.open_dataset(f'{mV.folder_save[0]}/tas/sample_data/{source}/{dataset}_tas_{mV.timescales[0]}_{experiment}_{mV.resolutions[0]}.nc')['tas'] if switch['sample_data'] else da
    da = gD.get_var_data(source, dataset, experiment, 'tas') if switch['gadi_data'] else da
    return da
