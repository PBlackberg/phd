...
def calc_daily_freq(switch, da_month, month, year, ws_values):
    ...
    for day, start in enumerate(range(0, len(da_month[da_month.dims[0]]), 8)):
        ...
        fig, _ = plot_scene(switch, scene, title = f'ws daily freq, day: {day}, month: {month}, year: {year}') if year == 2000 and month == 'january' and day == 0 else [None, None]
...
def calc_month_freq(switch, da_month, month, year, ws_values):
    ...
    fig, _ = plot_scene(switch, scene, title = f'ws month freq, month: {month}, year: {year}') if year == 2000 and month == 'january' else [None, None]
...
def get_metric(switch, metric):
    ...
    ws_values = [7, 8, 9, 10]      if switch['ws_lc'] else [7]                  # taken from examples of weather states from ISCCP (clouds below 600 hPa)
    ws_values = [1, 2, 3, 4, 5, 6] if switch['ws_hc'] else ws_values            # taken from examples of weather states from ISCCP (clouds above 600 hPa)
    ...
    for year in years:
        ...
        for month in ds.data_vars:
            ...
            if metric == 'sMean':
                metric_name =f'ws_lc_sMean' if switch['ws_lc'] else f'ws_hc_sMean'
            ...
            if metric == 'tMean':
                metric_name =f'ws_lc_tMean' if switch['ws_lc'] else f'ws_hc_tMean'
                ws_freq_tMean = calc_tMean_freq(da_month, ws_freq_tMean, ws_values) if mV.timescales[0] in ['monthly', 'daily'] else None
                da_calc = ws_freq_tMean / (len(years)*12)
                
    mF.save_in_structured_folders(da_calc, f'{mV.folder_save[0]}/metrics', 'ws', metric_name, source, dataset, mV.timescales[0], mV.experiments[0], mV.resolutions[0]) if switch['save'] else None
    ...
    # fig, _ = plot_scene(switch, ws_freq_tMean,  title = f'ws freq time mean') if mV.timescales[0] in ['monthly', 'daily'] and switch['tMean'] and switch['show'] else [None, None ]
