...
if __name__ == '__main__':
    ...
    for dataset in datasets:
        ...
        for experiment in experiments:
            ...
            if save:
                if in_descent_regions():
                    fileName = dataset + '_wap_snapshot_d_' + experiment + '_' + resolutions[0] + '.nc'
                    save_file(ds_snapshot, folder_save, fileName)

                    fileName = dataset + '_wap_tMean_d_' + experiment + '_' + resolutions[0] + '.nc'
                    save_file(ds_tMean, folder_save, fileName)
                    
                    fileName = dataset + '_wap_sMean_d_' + experiment + '_' + resolutions[0] + '.nc'
                    save_file(ds_sMean, folder_save, fileName)

                else:
                    fileName = dataset + '_wap_snapshot_' + experiment + '_' + resolutions[0] + '.nc'
                    save_file(ds_snapshot, folder_save, fileName)

                    fileName = dataset + '_wap_tMean_' + experiment + '_' + resolutions[0] + '.nc'
                    save_file(ds_tMean, folder_save, fileName)
                    
                    fileName = dataset + '_wap_sMean_' + experiment + '_' + resolutions[0] + '.nc'
                    save_file(ds_sMean, folder_save, fileName)
