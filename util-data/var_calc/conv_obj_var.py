...
def get_conv_threshold(da):
    conv_threshold = da.quantile(int(cD.conv_percentiles[0])*0.01, dim=('lat', 'lon'), keep_attrs=True)
...
def get_fixed_area_conv_threshold(da):
    return da.quantile(int(cD.conv_percentiles[0])*0.01, dim=('lat', 'lon'), keep_attrs=True)

def get_conv_var(switch, dataset, experiment, resolution):
    da = vB.load_variable({'pr': True}, switch, dataset, experiment, resolution, timescale = 'daily') #.isel(time = slice(0, 2))
    if switch.get('fixed_area', False): # if 'fixed_area' is an option in switch
        conv_threshold = get_fixed_area_conv_threshold(da) if switch['fixed_area'] else get_conv_threshold(da)
    else:
        conv_threshold = get_conv_threshold(da)
...
def get_conv_obj(switch, dataset, experiment, resolution):
    da = get_conv_var(switch, dataset, experiment, resolution)
    conv_obj = xr.zeros_like(da)
    obj_dim_size = len(da.lat) * len(da.lon)  # Upper limit of number of objects
    dim_list = []
...
if __name__ == '__main__':
    ...
    for timestep in np.arange(0, 1, 1):
        for dataset in mD.run_dataset_only(var = 'pr', datasets = cD.datasets):
            # -------------------------------------------------------------------------------------- Get data --------------------------------------------------------------------------------------------------- #
            conv = get_conv_var(switch, dataset, experiment)
            conv_obj, obj_id = get_conv_obj(switch, dataset, experiment)
