...
def plot_multiple_scatter(switch, var0, var1, title, datasets, timescale, resolution, folder_save):    
    ...
    for i, dataset in enumerate(datasets):
        ...
        pcm = ax.hist2d(x,y,[20,20], cmap ='Purples')
        color = 'purple'
...
def find_metric_and_units(plot_var):
    if plot_var['rlut']:
        variable_type = 'lw'
        axis_label = 'OLR [W m' + mF.get_super('-2') +']'
        region = name_region(plot_var)
        metric = f'rlut_sMean{region}'
...
def run_scatter_plot(switch, datasets, timescale, resolution, folder_save):
    print(f'Plotting rome and rlut correlation with {resolution} data')
    ...
    filename = f'{source}_rome_and_OLR{with_obs}'    
