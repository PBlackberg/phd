...
def get_list(switch, dataset, metric):
    ...
    if  metric.variable_type in ['pr', 'org']: # metrics from these variables are always on daily timescale and then resampled
        ...
        if dataset_alt == 'GPCP':
            path = f'{mV.folder_save[0]}/{metric.variable_type}/metrics/{metric_name}/{source}/{dataset_alt}_{metric_name}_{mV.conv_percentiles[0]}thPrctile_{timescale}_{experiment_alt}_{mV.resolutions[0]}_short.nc' if metric.variable_type == 'org' else path     
    ...
    alist = xr.open_dataset(path)[metric_option]

    # alist = xr.open_dataset(path)[metric_option].sel(time = slice('2000-03', '2021')) if dataset == 'CERES' else alist # full data between
    alist = xr.open_dataset(path)[metric_option].sel(time = slice('2010', '2021')) if dataset in ['ERA5', 'CERES', 'GPCP'] else alist # GPCP has separate trend for 2010-2022
...
def plot_one_dataset(switch, x, y, metric_x, metric_y, title = '', xmin = None, xmax = None, ymin = None, ymax = None):
    ...
    ax.set_xlim(xlims[mV.datasets[0]])
    ax.set_xticks(xticks[mV.datasets[0]])
    ax.set_xticklabels(xticks[mV.datasets[0]])
...
def plot_multiple_datasets(switch, metric_x, metric_y, title = '', axtitle = '', xmin = None, xmax = None, ymin = None, ymax = None):
    ...
    for i, dataset in enumerate(mV.datasets):
        ...
        ax.annotate('R$^2$: '+ str(round(res[0]**2,3)), xy=(0.2, 0.1), xycoords='axes fraction', 
                    xytext=(0.7, 0.05), textcoords='axes fraction', fontsize = 8, color = 'r') if res[1]<=0.05 else None
        # ax.set_xlim(xmin, xmax)
        # ax.set_ylim(ymin, ymax)
...
def run_scatter_trend_plot(switch):
    ...
    print(f'Plotting {keys[0]} and {keys[1]}, {mV.timescales[0]} correlation {ifWith_obs} \n with conv threshold at {mV.conv_percentiles[0]}th percentile of {mV.resolutions[0]} data')
    ...
    if switch['one dataset']:
        ...
        title = f'{mV.datasets[0]}_{metric_y.option}_and_{metric_x.option}{title}'
        title = f'{title}_fixed_area' if switch['fixed area'] else title
        fig = plot_one_dataset(switch, x, y, metric_x, metric_y, title)
        ...
        fig = plot_multiple_datasets(switch, metric_x, metric_y, title)
