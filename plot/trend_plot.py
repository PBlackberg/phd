...
def plot_multiple_datasets(switch, metric_x, metric_y, title = '', axtitle = '', xmin = None, xmax = None, ymin = None, ymax = None):
    ...
    for i, dataset in enumerate(mV.datasets):
        ...
        ax = axes.flatten()[i]
        x, _, _ = get_array(switch, dataset, metric_x)
        y, _, axtitle = get_array(switch, dataset, metric_y)
        ...
        mF.move_row(ax, 0.0875 - 0.025)            if row == 0 else None
        mF.move_row(ax, 0.0495 - 0.0135)            if row == 1 else None
        mF.move_row(ax, 0.01   - 0.005)              if row == 2 else None
...
def get_array(switch, dataset, metric):
    ...
    path = f'{mV.folder_save[0]}/{metric.variable_type}/metrics/{metric_name}/{source}/{dataset}_{metric_name}_{timescale}_{mV.experiments[0]}_{mV.resolutions[0]}.nc'
...
def run_scatter_trend_plot(switch):
    ...
    if switch['one dataset']:
        x, title, _ = get_array(switch, mV.datasets[0], metric_x)
        y, _, _ = get_array(switch, mV.datasets[0], metric_y)
        ...
        _, title, _ = get_array(switch, mV.datasets[0], metric_x)
