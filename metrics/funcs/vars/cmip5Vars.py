...
def get_pr(institute, model, experiment):
    ...
    path_folder =  os.path.join(path_gen, ensemble, version,'/pr')
    ds = concat_files(path_folder)
...
def get_tas(institute, model, experiment):
    ...
    version = os.listdir(os.path.join(path_gen, ensemble))[-1]
    path_folder =  os.path.join(path_gen, ensemble, version,'/tas')
    ds = concat_files(path_folder)

    tas = ds['tas']-273.15
    tas_n = regrid_conserv(tas)
    tas_n.attrs['units']= '\u00B0C'
...
def get_pw(institute, model, experiment):
    ...
    version = os.listdir(os.path.join(path_gen, ensemble))[-1]
    path_folder =  os.path.join(path_gen, ensemble, version,'/hus')
    ds = concat_files(path_folder)

    hus = ds['hus'].sel(plev=slice(850e2,0)).fillna(0) # free troposphere
    hus_n = regrid_conserv(hus)

    g = 9.8
    pw_n = xr.DataArray(
        data= -scipy.integrate.simpson(hus_n, hus_n.plev.data, axis=1, even='last')/g,
        dims=['time','lat', 'lon'],
...
def get_hur(institute, model, experiment):
    ...
    version = os.listdir(os.path.join(path_gen, ensemble))[-1]
    path_folder =  os.path.join(path_gen, ensemble, version,'/hur')
    ds = concat_files(path_folder)

    hur = ds['hur'].sel(plev=slice(850e2,0)).fillna(0) # free troposphere
    hur_n = regrid_conserv(hur)

    g = 9.8
    hur = xr.DataArray(
        data= -scipy.integrate.simpson(hur_n, hur_n.plev.data, axis=1, even='last')/g,
        dims=['time','lat', 'lon'],
...
def get_wap500(institute, model, experiment):
    ...
    wap500_n = regrid_conserv(wap500)
...
def get_clouds(institute, model, experiment):
    ...
    cloud_high = cloud_high.max(dim='plev')
