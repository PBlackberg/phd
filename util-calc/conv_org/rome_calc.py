...
def calc_q_weight(scene, labels, distance_matrix, dim):
    ...
    for i, label_i in enumerate(labels[0:-1]): 
        ...
        distance_from_obj_i = distance_matrix.sel(gridbox = n_list).min(dim = 'gridbox').compute()
...
def calc_rome_day(scene, labels_scene, distance_matrix, dim):
...
def get_rome(conv_obj, obj_id, distance_matrix, dim):
    ...
    for timestep in np.arange(0,2): #len(conv_obj.time.data)
        ...
        rome.append(calc_rome_day(scene, labels_scene, distance_matrix, dim))
...
if __name__ == '__main__':
    ...
    switch = {'test_sample': True, 'fixed_area': False}
    ...
    distance_matrix, _ = vC.get_variable(switch_var = {'distance_matrix': True}, switch = {'test_sample': False}, dataset = 'TaiESM1', experiment = experiment, resolution = cD.resolutions[0], timescale = cD.timescales[0], from_folder = True, re_process = False)
